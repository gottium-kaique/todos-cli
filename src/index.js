...
const showTodoTable = data => {
  ...
  data.map((todo, index) =>
    table.push([
      index,
      todo.title,
      todo.done ? chalk.green('Feito') : 'Pendente',
    ]),
  )
...
program
  ...
  .action(() => {
    shell.mkdir('src/backup')

    try {
      fs.renameSync('src/todos.json', 'src/backup/todos.json')
    } catch (err) {
      return console.log(chalk.red('Erro ao realizar backup!'))
    }
